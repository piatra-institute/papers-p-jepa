...
Run the integrated action-grounding challenge:
...
This is the current practical-use harness. It checks the full local workflow: passive-representation failure, action-consequence transfer, learned predicted-test representation, safe probe repair, learned restriction-map gluing, and skill composition. It remains a controlled benchmark, not a robot or foundation-model result.
...
Validate a robot/action manifest for future policy-learning evidence:
...
simulation/
├── pjepa_sim/
│   ├── benchmark/                    suite evaluator, P-JEPA stack, and action-grounding challenge
...
│   ├── verification/                 executable claim checks and evidence matrix
